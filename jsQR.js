...
function decodeMatrix(matrix, options) {
    ...
    // ★ Ver2.5：ユーザ暗号化はRS訂正後のデータ部だけをXOR解除する。
...
function resumeDecode(rawData, appMask) {
    try {
        var encryptedDataBytes = rawData.encryptedDataBytes || rawData.dataBytes || rawData.appEncDataBytesEncrypted;
        if (encryptedDataBytes && encryptedDataBytes.length) {
            var versionNumber = rawData.versionNumber || (rawData.version && rawData.version.versionNumber);
            if (!versionNumber) return null;
            var plainBytes = new Uint8ClampedArray(encryptedDataBytes.length);
            for (var di = 0; di < encryptedDataBytes.length; di++) {
                plainBytes[di] = encryptedDataBytes[di] ^ (appMask[di] & 0xFF);
            }
            var decoded = decodeData_1.decode(plainBytes, versionNumber);
            decoded.dataBytes = Array.from(plainBytes);
            decoded.appEncDataBytesEncrypted = Array.from(encryptedDataBytes);
            decoded.version = rawData.version;
...
function locate(matrix, options) {
    ...
    // QR Twin Ver5/Ver6 は位置検出点が細かく、1行飛ばしで初回探索から漏れやすい。
    // 後段の重い救済処理に回さないため、singleLocate でも全行を読む。
    var yStep = 1;
